...
以 FastAPI 应用为载体、在本地 k3s 集群上实测的完整部署链路：GitHub Actions 构建
多架构镜像 → ArgoCD GitOps 自动同步 → Prometheus/Grafana/Loki/Tempo 可观测 →
HPA 弹性伸缩与故障自愈。
...
- 🔁 **GitOps 闭环** — `git push` → GitHub Actions 构建多架构镜像并把新 tag 写回 chart → ArgoCD 自动同步到 K8s，Git 即集群状态真相源；回滚只需回退代码重新 push
...
- 🧰 **K8s 原生韧性机制验证** — HPA 自动扩缩容（2-10 副本，实测扩容至 6～7 副本）、PDB 防误删、Liveness/Readiness 分离探针自愈、金丝雀按权重灰度
...
> 压测的目的不是压应用性能，而是制造 CPU 压力触发 HPA 扩容、并在压力下验证可观测链路：
> 错误率 0%、trace 采集正常、副本按预期伸缩。首轮压测错误率 4.79% 的根因是 OTel 导出端点
> 配置错误（跨 namespace 短名解析失败，详见下方可观测性节），修复后归零——本身也是一次
> 完整的排障链路。
